...
async def test_subscribe_status(name, method, loop, exp_except, request):
    ...
    async def on_status(site: EliteCloudSite, section:EliteCloudSection, id:str, status: dict):
...
async def test_send_command(name, cmd_section, cmd_id, cmd_action, exp_status, exp_except, request):
    context = request.getfixturevalue("context")
    flags = {
        EliteCloudApiFlag.RESPONSE_DIVERT: True     # Also pass overal status messages as individual updates
    }
    ...
    # Define inline callback function
    async def on_status(site: EliteCloudSite, section:EliteCloudSection, id:str, status: Any):
    ...
    for site in sites:
        site_uuid = site.get('uuid')
        assert site_uuid is not None
        await context.api.send_site_command(site_uuid, cmd_section, cmd_id, cmd_action)
